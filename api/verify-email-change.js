...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!userId || !code || !newEmail) {
      return res.status(400).json({ error: "userId, code, and newEmail are required" });
    }

    const otpDoc = await db.collection("email_change_otps").doc(userId).get();
    ...
    await db.collection("email_change_otps").doc(userId).update({ used: true });

    return res.status(200).json({ success: true, message: "Email updated successfully" });
  } catch (error) {
    console.error("verify-email-change error:", error);
    return res.status(500).json({ error: error.message || "Failed to verify email change" });
  }
};
